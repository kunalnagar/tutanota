...
import o from "ospec"
import {compress, uncompress} from "../../../src/api/worker/lz4"
import {stringToUtf8Uint8Array} from "../../../src/api/common/utils/Encoding"

o.spec("Compression", function () {

	o.spec('empty', function () {
		o('should encode no data', function () {
			const empty = new Uint8Array(0)
			const a = compress(empty)
			const b = uncompress(a)
			const result = compress(b)
			o(Array.from(result)).deepEquals(Array.from(empty))
		})
	})
	...
	o.spec('small', function () {
		o.only('should encode no data', function () {
			...
			const b = uncompress(a)
			const result = compress(b)
			o(Array.from(result)).deepEquals(Array.from(small))
		})
	})
